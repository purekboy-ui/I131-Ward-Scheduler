...
| 關鍵字 | 建議 Skill | 原因 |
|--------|------------|------|
| 錯誤、error、exception、bug、crash、報錯 | `systematic-debugging.skill.md` + `error-handling.skill.md` | 啟動 4 階段除錯流程 |
| log、日誌、monitor、trace、root cause | `systematic-debugging.skill.md` | Log 分析與根因追蹤 |
| fix、修復、壞了、not working | `systematic-debugging.skill.md` | 避免 Trial and Error |
...
偵測到以下專案結構時，自動建議對應 workflow/skill：

| 偵測條件 | 專案類型 | 建議動作 |
|----------|---------|---------|
| `manifest.json` 存在 | Chrome Extension | 判斷 v2/v3，建議 `chrome-extension.md` |
| `ios/` 或 `*.xcodeproj` 存在 | iOS | 確認 Swift/ObjC、Deployment Target |
| `android/` 或 `build.gradle` 存在 | Android | 確認 Kotlin/Java、minSdk/targetSdk |

> 偵測到類型但缺少對應 workflow：**只提醒**可建立，不可自動建立。
