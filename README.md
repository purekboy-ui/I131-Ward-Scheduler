...
| 後台管理 | 醫師、床位開關、帳號 | 單日切換與日期區間批次調整集中處理，減少反覆點日期 |
...
- **作用**：集中維護主治醫師、床位開關、帳號，並支援床位單日微調與日期區間批次套用。
- **為什麼這樣設計**：
  - 避免後台出現大片無意義留白。
  - 三種管理工作集中在同一頁，減少切頁與找功能的成本。
  - 單日切換留給臨時調整，批次套用留給長假、整段關床或固定星期開關。
...
4. **一般使用者登入後只保留「預約排程」頁面，管理頁面只給管理者看**。
5. **床位是否可排是日期 x 床位開關，不是只看星期**。
6. **管理者可用日期區間 + 星期條件批次開關床位，避免逐日重複操作**。
7. **門診小劑量預設可排任何未來日期**。
8. **主治醫師清單會同步反映在後台、預約表單、報表篩選**。
...
| 帳號 | 密碼 | 角色 | 用途 |
| --- | --- | --- | --- |
| `admin` | `admin` | 管理員 | 檢視完整功能、收單、後台維護與批次床位管理 |
| `user` | `user` | 一般使用者 | 只看預約排程，並驗證排程與鎖定規則 |
...
Repo 目前已收斂成展示所需的 live 檔案與交接文件，不再保留未使用的舊模組檔。
